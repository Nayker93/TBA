...
Que se passe t-il si le joueur entre une commande qui n’est pas valide ? 

Si le joueur entre une commande qui n'est pas valide, il ne se passera rien. Car dans game.py (l.91-92), si la commande détectée n'est pas dans self.commands.keys() alors cela retourne None.

Que se passe t-il si le joueur entre une commande qui n’est pas complète ? 

Si le joueur entre une commande qui n'est pas complète, alors cela affiche un message d'erreur dans actions.py en utilisant la String MSG0 ou MSG1 en fonction de la méthode de la commande prenant des paramètres ou non.

Identifier les parties du code qui gèrent ces cas et observer attentivement comment ces cas sont traités.

En analysant la commande `go`, on peut observer que si on écris une directon qui n'est pas dans le dictionnaire exits de la pièce courante, alors cela affichera un message d'erreur KeyError: 'La touche utilisée'.

## Exercice 1 : Gestion d'une direction inconnue

Quelle est la classe dans laquelle est définie la commande go ?
